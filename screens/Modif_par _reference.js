...
class Modif_par_reference extends React.Component{
    ...
    async reference(value){
        ...
        if(table.includes(this.state.choixentrpse)){
          ...
          let references=[];
          const ventes = await requete.fetchreference(this.state.choixentrpse,this.state.choixprdt);  
          if(ventes.length>0){
        ...
        /*let array= this.state.ventes.map((item) => item.nom);
        let references= [];
        console.log('vvvvvvvvvventes',this.state.ventes);
        
        if(array.includes(value)==true){

            this.state.ventes.map((item) => (
               value==item.nom && this.state.choixentrpse==item.raison_social ? 
               references.push({  label: `${item.reference} du ${item.date_vente}` ,value: `${item.reference} du ${item.date_vente}`}):
               {label:'', value:''}
            ));
            this.setState({references:references});

        }
        else if(array.includes(value)==false && this.state.choixprdt!="null"){
            
            references.push({label:"pas de reference", value:"pas de reference"});
            this.setState({references:references});
            Alert.alert("Desole ce produit/service n'a jamais ete vendu!!!");
        }
            */
    ...
    render(){
        ...
        //console.log("referenceee"+JSON.stringify(params));
